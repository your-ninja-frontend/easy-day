...
const findIndex = (state: RootState, action: PayloadAction<{ id?: string; done?: boolean; listId?: string }>) =>
...
const rootSlice = createSlice({
  ...
  reducers: {
    setTodoStatus: (state, action: PayloadAction<{ id: string; done: boolean; listId: string }>) => {
      const indexList = findIndex(state, action);
      const indexTodo = state.todoLists[indexList].todos.findIndex((todo) => todo.id === action.payload.id);
    ...
    removeTodo: (state, action: PayloadAction<{ id: string; listId: string }>) => {
      const indexList = findIndex(state, action);
      state.todoLists[indexList].todos = state.todoLists[indexList].todos.filter(
        (todo) => todo.id !== action.payload.id
      );
    },
    changeListTitle: (state, action: PayloadAction<{ title: string; listId: string }>) => {
      const indexList = findIndex(state, action);
      state.todoLists[indexList].title = action.payload.title;
    },
    changeTodoTitle: (state, action: PayloadAction<{ id: string; listId: string; title: string }>) => {
      const indexList = findIndex(state, action);
      const indexTodo = state.todoLists[indexList].todos.findIndex((todo) => todo.id === action.payload.id);
    ...
    addTodo: (state, action: PayloadAction<{ listId: string; value: string }>) => {
      const indexList = findIndex(state, action);
    ...
    changeDate: (state, action: PayloadAction<{ listId: string; date: string }>) => {
      const indexList = findIndex(state, action);
